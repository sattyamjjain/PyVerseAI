...
def list_session(auth: Auth) -> List[dict]:
    ...
    for sess in _sr.list() if auth.role == 'STUDENT' else _sr.get(dean_name=auth.name):
        ...
        if sess_as_dict['is_available'] and __is_session_available(sess_as_dict.get('end_time')):
...
def book_slot(_id: int, st_name: str, start_time: int, end_time: int = None):
    ...
    if end_time:
        ...
        if end_time < start_time or end_time - start_time != datetime.timedelta(hours=1):
            raise AssertionError('End time should be in difference of max 1 hour')
    else:
        end_time = start_time + datetime.timedelta(hours=1)
    if start_time.strftime('%A') not in ['Thursday',
                                         'Friday'] or start_time.hour != 10 or start_time.minute != 0 or start_time.second != 0:
